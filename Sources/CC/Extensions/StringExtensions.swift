...
public extension String {
  ...
  func paths(using font: CTFont, flip: Bool = String.defaultPathFlip) -> [CGPath] {
    var paths: [CGPath] = []
    ...
    for run in runs
    {
      ...
      for index in 0..<count
      {
        ...
        if let letterPath = CTFontCreatePathForGlyph(font, glyph, nil) {
          ...
          if flip {
            let letterPathBounds = letterPath.boundingBoxOfPath
            transform = transform.scaledBy(x: 1, y: -1)//.translatedBy(x: 0, y: letterPathBounds.height)
          }

          paths.append(letterPath.copy(using: &transform) ?? letterPath)
        }
      }
    }
